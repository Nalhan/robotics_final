...
thd = [th1d th2d th3d th4d th5d th6d]';
...
gs30 = [eye(3) [0       0  0]';0 0 0 1];
gs40 = [eye(3) [-w/2    0 r0]';0 0 0 1];
gs50 = [eye(3) [w/2    r1 L0]';0 0 0 1];
gs60 = [eye(3) [w/2 L1+r2 L0]';0 0 0 1];
...
M6 = [m(6)*eye(3) zeros(3);zeros(3) [Ix(6) 0 0;0 Iy(6) 0;0 0 Iz(6)]]
...
z5 = twistr(w5,q5);
z6 = twistr(w6,q6);
z  = [z1 z2 z3 z4 z5 z6];              % put in list for access later
...
z6_p = simplify(Ad(expt(z1,th1)*expt(z2,th2)*expt(z3,th3)*expt(z4,th4)*expt(z5,th5))*z6)

Jsst = [z1 z2_p z3_p z4_p z5_p z6_p]
...
gs6th = simplify(expt(z1,th1)*expt(z2,th2)*expt(z3,th3)*expt(z4,th4)*expt(z5,th5)*expt(z6,th6)*gs60);
...
Mp(6).M = sym(Ad(ginv(gs60))'*M5*Ad(ginv(gs60)));
...
V = g*[m(1) m(2) m(3) m(4) m(5) m(6)]*[cm1(3) cm2(3) cm3(3) cm4(3) cm5(3) cm6(3)]'   %summation of mgh for all 3 links
...
matlabFunction(gs6th,'file','gs6func','vars',{x})
...
matlabFunctionBlock('mobilebot_Model/gs4th',gs4th,'vars',{x})
matlabFunctionBlock('mobilebot_Model/gs5th',gs5th,'vars',{x})
matlabFunctionBlock('mobilebot_Model/gs6th',gs6th,'vars',{x})
...
matlabFunctionBlock('mobilebot_Model/Jsstfunc',Jsst,'vars',{x})
